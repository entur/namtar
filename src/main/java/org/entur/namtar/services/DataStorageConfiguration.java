...
public class DataStorageConfiguration {
    ...
    public DataStorageService createDataStorageService() throws IOException, InterruptedException {
        Datastore service;
        String google_application_credentials = System.getenv(DatastoreOptions.CREDENTIAL_ENV_NAME);
        if (StringUtils.isNotBlank(google_application_credentials)) {
            service = DatastoreOptions.getDefaultInstance().getService();
            logger.info("Found GOOGLE_APPLICATION_CREDENTIALS as environment variable and instantiates a Datastore service based on those!");
        } else {
        ...
        DataStorageService dataStorageService = new DataStorageService(service);
